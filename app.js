...
app.get('/cond-comerciales', (req, res) => res.sendFile(path.join(__dirname, 'public', 'comercial.html')));
...
app.post('/subir', upload.fields([
  ...
  if (password !== 'cuadrobonito11') {
    return res.status(403).send('<p>Contraseña incorrecta. <a href="/subir.html">Volver</a></p>');
  }
  // Verificar que ambos archivos estén
  if (!req.files || !req.files.stock_cba || !req.files.stock_olavarria) {
    return res.status(400).send('<p>Debe subir ambos archivos. <a href="/subir.html">Volver</a></p>');
  }
...
app.get('/last-update', (req, res) => {
  if (!fs.existsSync(updateLog)) {
    return res.status(404).json({ error: 'No se ha registrado ninguna actualización aún.' });
  }
